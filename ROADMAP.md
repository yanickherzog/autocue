...
**Status: Locally verified (2026-08-10), CI and manual click-through still pending.** `swift build`/`swift test` clean across all 7 packages (`ACCore` 176 tests, `ACFeatures` 22, `ACTestSupport` 31, others unchanged), plus a real `xcodebuild build`/`test -scheme AutoCue` (13 `AutoCueTests`, all green) — zero compiler warnings, `swiftformat --lint`/`swiftlint --strict`/both architecture-boundary scripts all clean. **Not yet done, flagged here rather than silently claimed:** a real, watched-to-completion CI run on the pushed branch (this project's own standing bar, not local verification alone), and the user's own interactive click-through of the Acceptance Criteria below — this session confirmed the app builds, launches, and reaches the Setup screen (screenshot-verified: Library empty state renders correctly), but has no Accessibility/Input Monitoring permission to drive real field-by-field interaction (typing, picking a party, toggling checkboxes, triggering the delete-guard message), the same environment limitation `docs/REVIEW.md`'s D6 entries already documented.
...
- **T7.3 — Right-holder directory and party picker** *(was M15)*. `Packages/ACFeatures/Sources/ACFeatures/CueSheetEditor/ViewModels/RightHolderDirectoryViewModel.swift`, `Views/{PersonEditorSheet,LabelEditorSheet,PartyPickerView,PostalAddressFields}.swift` — `PostalAddressFields.swift` moved here from T7.2's original file list: its real consumers are `PersonEditorSheet`/`LabelEditorSheet` (`Person.address`/`Label.address`), not `SetupView` directly (`Setup` has no `PostalAddress` field of its own).
...
**Documentation Requirements:** `docs/DECISIONS.md` gained two entries (the schema correction, the synchronous-factory-vs-async-fetch pattern) — more than "none expected," since real, debatable architectural calls surfaced during this Deliverable that its original scope didn't anticipate.
