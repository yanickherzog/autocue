...
**Status: Complete (2026-08-10), pending the user's own manual click-through and a real CI run.** Verified locally: `swift build`/`swift test` × 7 packages (0 failures — `ACCore` 158, `ACPersistence` 19, `ACFeatures` 4, `ACTestSupport` 20, `ACDesignSystem` 5, `ACAudioKit` 1, `ACExport` 4); `swiftformat --lint` (0 files needing formatting, 121 checked); `swiftlint lint --strict` (0 violations, 121 files); both architecture-boundary scripts; a real `xcodebuild -workspace AutoCue.xcworkspace -scheme AutoCue build`/`test` run (Xcode 26.3) — BUILD SUCCEEDED, TEST SUCCEEDED, 7/7 `AutoCueTests` passing, zero warnings. The built `.app` was launched directly (not just built): `codesign -d --entitlements` confirmed `com.apple.security.app-sandbox = true` is actually embedded, and the running process produced no sandbox-violation/denial entries in the system log. **Not yet done:** the user's own interactive walkthrough (create a named project, see it listed, open/focus its window, delete it) — this session has no GUI-automation tool to drive that itself, so it's verified here only as far as "the app launches and runs cleanly," not the full manual acceptance criterion below; and a real, watched-to-completion CI run on the actual pushed branch, per this project's own standing practice of treating that as the real bar, not local success alone.
