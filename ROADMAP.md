...
- **T1.4 — Core value types** *(was M2)*. `MediaDuration`, `Timecode`, `TimecodeFrameRate` already implemented and tested ahead of schedule (`Packages/ACCore/Sources/ACCore/Models/`, `SPEC.md` §4.8–§4.9, `docs/DECISIONS.md`) — this task's remaining scope is `PostalAddress`, `Party`, and `ProgressUpdate`/`OperationProgress` (`SPEC.md` §4.17), plus folding the existing `ACCore`/`ACExport` spike packages into the real T1.1 workspace structure.
