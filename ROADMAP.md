...
**Schema-correction prerequisite, completed ahead of this Deliverable's own Tasks:** cross-checking D7's planned UI against `SPEC.md` §4.2 and the original product brief surfaced three fields missing from the finalized schema (`Setup.beitrag`/`.exploitationTypes`/`.broadcastDetails`) and one role missing from `CueRightHolderRole` (`.performer`, reversing §2.2's original performer-exclusion decision) — added in their own commit, separate from D7's feature work per `CONTRIBUTING.md` §4. See `docs/DECISIONS.md` for both entries and `SPEC.md` §4.2–§4.2.4/§4.4/§2.2 for the corrected schema.

**Tasks:**
- **T7.1 — Setup view model and use case** *(was M13)*. `Packages/ACCore/Sources/ACCore/UseCases/UpdateSetupUseCase.swift`; `Packages/ACFeatures/Sources/ACFeatures/CueSheetEditor/ViewModels/SetupViewModel.swift`. No UI yet. Also, not in this Task's original file list but required by `CONTRIBUTING.md` §6's "ViewModels call Use Cases only" (no Use Case existed for `Person`/`Label` directory create/update): `Packages/ACCore/Sources/ACCore/UseCases/UpdateRightHolderDirectoryUseCase.swift`.
- **T7.2 — Setup view** *(was M14)*. `Packages/ACFeatures/Sources/ACFeatures/CueSheetEditor/Views/SetupView.swift`, `ProductionTypePicker.swift`. Also, not in this Task's original file list: `AttachmentTypePicker.swift` (`Setup.attachmentTypes`/`.otherAttachmentDescription` need UI too, same small checkbox-group shape as `ProductionTypePicker` — two instances doesn't clear `CONTRIBUTING.md` §3's rule-of-three bar for extraction into one shared component, so built separately).
