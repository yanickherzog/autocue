...
**Status: Locally verified after a first real manual click-through round (2026-08-10), CI and a second click-through still pending.** The user's own click-through of the first build found a large number of real bugs and one structural design gap — not a clean first pass, matching D6's own precedent of needing multiple rounds. Fixed in this same pre-push state: missing-field validation showing prematurely on an untouched screen (now gated on `SetupViewModel.hasStartedEditing`); ghost/placeholder text, a `List`, and a `Picker`'s selected-value display all rendering invisibly under this session's actual system Dark Mode (root-caused via a real live-windowed test app, not guessed — see `docs/DECISIONS.md`'s `fixedAppearance` entry); a real state-update race in `PartyPickerView` (a redundant, second `loadDirectory()` subscription that could apply a stale snapshot after a concurrent save); `Setup.producer`/`.directorOrPrincipal`/`.declarant` sharing one directory confirmed correct per SPEC.md (not a bug), while the actual gap — no dedicated collaborator-roster screen — closed with a real, if small, schema addition (`Person.intendedRole`, see `docs/DECISIONS.md`); Total Music Runtime removed from this screen (belongs on Review & Export); `DatePicker` stepper arrows removed; `AttachmentTypePicker` hidden from the UI (optional, unused in practice, schema kept intact); `containsAdditionalUndeclaredWorks`'s default changed to `.no`. `swift build`/`swift test` clean across all 7 packages, plus a real `xcodebuild build`/`test -scheme AutoCue` — zero compiler warnings, `swiftformat --lint`/`swiftlint --strict`/both architecture-boundary scripts all clean. **Not yet done:** a real CI run on the pushed branch, and the user's own second click-through confirming these fixes — this session verified what it could without Accessibility permission (a real live-windowed test app + screenshots for every color/rendering fix, confirmed working; the app relaunched and left running for the user each round), consistent with `docs/REVIEW.md`'s D6 precedent for this same environment limitation.
...
- **Post-click-through fixes** *(this round, not a new Task)*. `Packages/ACDesignSystem/Sources/ACDesignSystem/Modifiers/FixedAppearanceModifier.swift` (native-control appearance fix, see `docs/DECISIONS.md`).
...
**Documentation Requirements:** `docs/DECISIONS.md` gained five entries total across both rounds (the schema correction; the synchronous-factory-vs-async-fetch pattern; the `fixedAppearance` native-control finding; `Person.intendedRole`'s roster redesign; the `containsAdditionalUndeclaredWorks` default change) — more than "none expected," since real, debatable architectural calls surfaced during this Deliverable that its original scope didn't anticipate. `SPEC.md` §4.5 updated for `Person.intendedRole` per rule 9.
