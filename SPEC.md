...
- CI (`.github/workflows/ci.yml`) exists as of `ROADMAP.md` D1. Its pinned Xcode version was flagged as unconfirmed against a live runner in an earlier revision of this document; it has since actually been run and genuinely failed once (15.4's Swift 5.10 toolchain vs. a then-declared `swift-tools-version: 6.1`), diagnosed from real CI logs, and fixed by lowering the tools-version declaration rather than the Xcode pin — see `docs/DECISIONS.md`. Closed, not still open.
