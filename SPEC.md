...
**RMS time resolution: hop size and threshold-crossing interpolation — closing the gap between the 50ms window and the 1-frame target.** "Boundary accuracy target," below, requires locating a boundary within 1 frame of the project's configured `Setup.timecodeFrameRate` — between roughly `33.3`ms (`.fps30`/`.fps29_97NonDrop`/`.fps29_97Drop`) and `41.7`ms (`.fps24`) depending on frame rate. Taken at face value, `analysisWindowMilliseconds`'s `50.0`ms default is coarser than this target. For cue *starts* this doesn't matter in practice — SuperFlux's stage 2 (below) relocates the boundary directly from the spectrogram, at a time resolution far finer than any RMS window, so stage 1's own precision is masked by stage 2 regardless. **For cue *ends* it matters completely**, because offsets have no equivalent second stage (see "Why offset detection does not use SuperFlux or any onset-detection technique," below) and are finalized entirely by this RMS mechanism — so the RMS mechanism's own resolution *is* the offset-accuracy budget, with nothing downstream to correct for it. Two additions, both plain RMS/energy-domain techniques — consistent with offsets deliberately staying off any onset-detection-family (spectral/novelty) technique — close this gap:
...
5. **Peak-picking.** Within the search window, the strongest peak on this frame-by-frame novelty function is located. If its prominence clears `superFluxOnsetSensitivity` (normalized `0`...`1`), the frame it occurs at becomes the refined boundary.

**This does not replace stage 1 and does not run standalone** — it only ever refines a boundary stage 1 has already roughly located, narrowing an arbitrary level-crossing point down to the actual moment of musical/spectral change. **Fallback:** if no novelty peak clears `superFluxOnsetSensitivity` anywhere within the search window (e.g. a steady tone with no meaningful spectral change crossing the RMS threshold), stage 2 leaves the stage-1 boundary unchanged rather than relocating it to a spurious or arbitrary peak — the same "fall back to the simpler, already-defined behavior when refinement is inconclusive" pattern `noiseFloorCalibrationMode`'s automatic-mode fallback already establishes, above.
