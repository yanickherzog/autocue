...
| `spectralFluxRefinementSearchWindowSeconds` | `Double` | `0.5` | stage 2 only: how far, in seconds, before/after the stage-1 (RMS-threshold) candidate boundary the spectral-flux onset search looks — see "Spectral flux-based onset refinement," below. Bounds how far refinement can relocate a boundary from its stage-1 candidate; not a general search radius |
| `spectralFluxOnsetSensitivity` | `Double` | `0.3` | stage 2 only: normalized (`0`...`1`) peak-picking sensitivity for identifying the onset frame on the spectral flux curve within the search window above; higher values treat subtler spectral changes as the onset. Not a dBFS value — spectral flux has no natural dBFS scale, unlike `silenceThresholdDb` |
...
**Spectral flux-based onset refinement (stage 2, precise timing only — not a semantic classifier).** RMS-threshold detection has a specific, known weakness: for a cue that fades in gradually or is mixed in deliberately quiet, there is no single clean moment where the level crosses any threshold, since the level ramps up by construction — wherever the threshold sits, the crossing point is somewhat arbitrary relative to where a human editor would actually place the cue's start, and tends to land later than the true onset. Spectral flux — the frame-to-frame change in FFT magnitude spectrum, a standard, well-established MIR (music information retrieval) onset-detection technique, computed via `Accelerate`'s existing vDSP FFT support — addresses this directly: within the candidate region stage 1 already identified (bounded by `spectralFluxRefinementSearchWindowSeconds` on either side of the stage-1 boundary), stage 2 computes spectral flux frame-to-frame and picks the strongest onset peak within that window, at `spectralFluxOnsetSensitivity`'s sensitivity, as the refined boundary. **This does not replace stage 1 and does not run standalone** — it only ever refines a boundary stage 1 has already roughly located, narrowing an arbitrary level-crossing point down to the actual moment of musical/spectral change. **Fallback:** if no spectral flux peak clears `spectralFluxOnsetSensitivity` anywhere within the search window (e.g. a steady tone with no meaningful spectral change crossing the RMS threshold), stage 2 leaves the stage-1 boundary unchanged rather than relocating it to a spurious or arbitrary flux peak — the same "fall back to the simpler, already-defined behavior when refinement is inconclusive" pattern `noiseFloorCalibrationMode`'s automatic-mode fallback already establishes, above. **Scope, deliberately bounded to cue starts:** this stage refines *silence→sound* (cue-start) boundaries only, where spectral flux's onset-detection strength directly applies. A cue's *end* into a decaying tail is a different, already-solved problem — bounded by `tailToleranceDb`/`tailCapSeconds` below — and is not in scope for spectral-flux refinement; onset detection is not naturally suited to offset/decay timing, which is a distinct MIR problem this contract doesn't need to solve. **Explicitly out of scope: speech/music discrimination and any ML-based/trained classification.** This app's audio imports are music-only — never dialogue or sound design — so there is no music-vs-speech ambiguity for `SilenceDetector` to resolve, and no classifier of any kind is needed. This was a real technique considered during the research that led to adding spectral flux, and deliberately ruled out as inapplicable to this app's actual input material — not an oversight. See `docs/DECISIONS.md`, 2026-08-13, for the full record of both the addition and this scope boundary.
