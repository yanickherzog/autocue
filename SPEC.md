...
- CI (`.github/workflows/ci.yml`) exists as of `ROADMAP.md` D1. Its pinned Xcode version was flagged as unconfirmed against a live runner in an earlier revision of this document; it has since actually been run twice and genuinely failed both times — once on `swift-tools-version` (15.4's Swift 5.10 toolchain vs. a then-declared `6.1`), once on `.swiftformat`'s matching `--swiftversion` setting producing trailing commas in function calls that same Swift 5.10 compiler can't parse — both diagnosed from real CI logs, not guessed, and both fixed by correcting the declared version to match the real floor rather than moving the Xcode pin. See `docs/DECISIONS.md` for the full sequence, including a correction to that log's own initial "confirmed" claim, which was written before the second failure was known. Treat as closed only once a subsequent CI run is actually observed green.
