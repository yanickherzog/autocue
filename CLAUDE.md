...
**Toolchain implication — corrected, not just updated:** targeting the macOS 14 SDK only technically requires Xcode 15+ to compile at all, but that is **not** this project's real minimum build toolchain. **Xcode 26.3 is required.** Xcode 15.4 — this project's own original CI pin, chosen specifically because it matched the SDK-level "15+" figure above — has a real, confirmed defect in SwiftData's handling of an ordinary `@Model` to-many relationship with an inverse (a `SIGTRAP`/`EXC_BREAKPOINT` crash inside SwiftData's own macro-generated relationship accessor, on correct, unremarkable relationship code, not a mistake in this project's schema). Confirmed deterministically on real CI runs and independently reproduced in a toy `@Model` pair with zero connection to any of this project's actual entities — see `docs/DECISIONS.md`, "D4's real, confirmed root cause" (2026-08-09), for the full investigation, evidence, and the CI fix. **Only Xcode 26.3 has actually been verified clean** — no version between 15.4 and 26.3 has been tested, so this is a confirmed-safe floor, not necessarily the tightest possible one; narrowing it further is a reasonable future exercise, not a blocker. This is a development-machine/CI-toolchain requirement, unrelated to the app's own "runs fully offline" constraint (Technology Stack, above), which governs the shipped app's runtime behavior, not the build toolchain used to produce it, and it does not change the macOS 14.0 *deployment* target above — a shipped, notarized build still only needs to run on macOS 14+, regardless of which Xcode version built it.

**Enforced at:** every package's `Package.swift` (`platforms: [.macOS(.v14)]` — already applied consistently in `Packages/ACCore/Package.swift` and `Packages/ACExport/Package.swift`, the two packages that exist ahead of schedule per earlier architecture-correction work) and the App target's deployment target setting, once it exists (`ROADMAP.md` D1). `ROADMAP.md` D1's acceptance criteria and D15's packaging pass both reference this section. The Xcode-26.3 toolchain floor is enforced in CI via `.github/workflows/ci.yml`'s `xcode-version` pin (all three jobs, `macos-15` runner) — a contributor building locally with Xcode 15.4 will not just be "on an older version," they will hit the real crash described above.
...
- **`Cue`** — many per `Project`, ordered. One SUISA "musical work" entry: title, work number, `MediaDuration`, and its `[CueRightHolder]` list, plus `isArrangementOfProtectedOriginal: Bool` — not itself printed on the form, but not a purely app-internal workflow aid either, since it's what determines whether `CueRightHolder.arrangementAuthorizationAttached` is required for any `.arranger`-role right-holder on the work (SPEC.md §4.3–§4.4, §4.6). Carries a few purely app-internal-only fields (`source`, `startTimecode`, `notes`) used for the auto-detection workflow — these are never exported to the SUISA document, since SUISA wants total usage duration per work, not on-screen position or usage category.
- **`CueRightHolder`** — sub-entity of `Cue`. A `Party` (composer/author/arranger/publisher, plus app-only `.performer` — informational, excluded from both share sums and the PDF export; see `SPEC.md` §4.4, §2.2) plus performance-broadcast and mechanical-rights percentage shares. Shares must sum to 100% per Cue, per share type — validated in a Use Case, never in the struct.
...
The concrete visual design AutoCue targets — stated here for the first time in this document because `SPEC.md` §3 correctly scopes UI/visual design out of that file, and nothing enforced writing it down anywhere else before `ROADMAP.md` D5 (Design System Foundations) actually needed it. **These are the project's original visual-design decisions, not inventions of D5** — D5's job was building the tokens/components that implement them, not deciding what they are.

**Fixed appearance — AutoCue does not adapt to the system Light/Dark Mode setting.** This is a deliberate departure from the usual macOS convention of `Color(light:dark:)`-style adaptive tokens, not an oversight or a gap to fill with light/dark counterparts. See `docs/DECISIONS.md` for the full record of this choice, including why it was chosen over standard system-appearance adaptation.

**Color palette** — three fixed values, identical regardless of system appearance:

| Token | Hex | Role |
|---|---|---|
| Carbon Black | `#202020` | primary text/foreground on light surfaces; background on reversed surfaces |
| White | `#FFFFFF` | primary background on light surfaces; foreground on reversed surfaces |
| Burgundy (accent) | `#93032E` | accent color, constant across both surface styles below |

**Two fixed surface styles, chosen per screen — not per system appearance.** Setup and Cue Sheet use the **primary** surface (White background, Carbon Black foreground); Review & Export uses the **reversed** surface (Carbon Black background, White foreground). Burgundy is the accent on both. `ACDesignSystem/Theme/Colors.swift` exposes these as `Theme.Surface.primary`/`.reversed` (a sibling of `Theme.Colors`, not nested inside it — SwiftLint's `nesting` rule caps type nesting at 1 level) plus `Theme.Colors.accent` — never as a `colorScheme`-driven pair.

**Ghost/placeholder text:** Carbon Black at **40% opacity**, on the primary surface only. Placeholder text is de-emphasized by opacity alone, not by font weight — see Typography, below, for why no lighter weight is bundled for this purpose. No reversed-surface equivalent is defined yet; nothing in `SPEC.md`/`ROADMAP.md` currently puts an input field on a reversed-surface screen. Add one only when a real screen needs it (rule 7), not speculatively.

**Typography:** Space Grotesk (Google Fonts, SIL Open Font License), bundled as static weights — Regular, Medium, Bold — vendored into `ACDesignSystem/Sources/ACDesignSystem/Resources/Fonts/` (plus `OFL.txt`) and registered at runtime via `CTFontManagerRegisterFontsForURL` off `Bundle.module`. No Light weight is bundled: ghost text's de-emphasis is carried entirely by the 40%-opacity color token above, not by weight — stacking a second de-emphasis signal on top would risk placeholder text visually shifting weight the moment the user starts typing (typed text renders Regular), which reads as inconsistent rather than as a feature. Fetching the font files at implementation time from `google/fonts` is a build/dev-time action, vendored into the repo — not a runtime network dependency, the same distinction already established for `libxlsxwriter`'s remote SPM fetch (see "Export Architecture," below).

**Registration is automatic, lazy, and idempotent — not a forward dependency any call site has to remember.** `Theme.Typography.font(_:size:)` triggers `registerFonts()` on first use, guarded by a thread-safe `static let`, so both SwiftUI Previews and the real running app register the font the first time any token is actually rendered, with no explicit call required anywhere (including D6/T6.1's `AutoCueApp.swift`). `registerFonts()` stays a public method for the rare case an early, explicit call is wanted (e.g. to front-load registration before first paint); calling it more than once is a harmless no-op.

**Sharp corners — no rounded corners anywhere, ever.** No button, container, card, or input field is ever rounded. Enforced as a real, reusable component (`Modifiers/SharpButtonStyle.swift`), not left as a prose guideline any future screen could silently violate — SwiftUI's stock `.bordered`/`.borderedProminent` button styles round corners by default on macOS, so this needs an explicit style, not just "don't add `.cornerRadius()`."

**Dividers:** simple straight lines (SwiftUI's stock `Divider()`), colored from `Theme.Colors` rather than the system default gray — no new component needed beyond a token reference, since `Divider()` already renders exactly "a simple line."

**Transitions:** a single shared ~1.5s smooth transition, `Theme.Motion.standard` (`Animation.easeInOut(duration: 1.5)`) plus a `.standardTransition()` `ViewModifier`, used for tab switches and hover-state changes. Every screen from D6 onward reuses this one constant — never a per-screen reimplementation of "the transition," the same standing-convention discipline already established for `WaveformDisplayData`/`CueTableView`'s row protocol above.

**Default window size:** 1290×800, defined as `Theme.Layout.defaultWindowSize` (`ACDesignSystem/Theme/Spacing.swift`) even though its only consumer is `ROADMAP.md` D6/T6.1's `AutoCueApp.swift` scene declaration — defined once now so D6 references a token instead of a hardcoded magic number. Layout is responsive to resizing where feasible; 1290×800 is the default/initial size, not a fixed constraint.
...
**Reader/writer barrier — per-`Project.ID` write serialization above is not the whole concurrency story.** `writeTails` only serializes writes against *other writes for the same `Project.ID`*; it says nothing about `ProjectRepositoryImpl`'s all-projects snapshot fetch (used after every write, and on new subscription), which touches *every* project's entity graph — including ones with no relationship to whichever write just completed. A real crash was reproduced (2026-09-05, live under a debugger, see `docs/DECISIONS.md`): one project's snapshot fetch was mid-fault-resolution on a *different* project's entity at the exact moment that other project's own, entirely independent write deleted and reinserted it — SwiftData's own "invalidated because its backing data could no longer be found in the store." Different `Project.ID`s are deliberately allowed to write concurrently (that's the point of the per-ID scheme above), so nothing previously prevented that overlap.

Fixed with a real reader/writer barrier (`Packages/ACPersistence/Sources/ACPersistence/ProjectRepositoryImpl+ReaderWriterBarrier.swift`), not a return to global write serialization — that alternative was rejected specifically because it would reintroduce the exact cross-window blocking the multi-window design above exists to avoid. The barrier's actual rule: no all-projects snapshot fetch may run while *any* write, for *any* `Project.ID`, is in flight; no write may start while a snapshot fetch is in flight. Writer-vs-writer concurrency for different `Project.ID`s is fully preserved — `writeTails` is untouched by this. The only new blocking is writer-vs-reader, and both sides of it are brief, one-shot conditions in this app's actual usage (a fetch runs once per write or once per new subscription, never as a standing operation) — not a bottleneck. A same-`Project.ID` write's own post-write snapshot fetch is folded into the same `ModelContext`/write unit rather than going through the barrier at all, closing the narrower same-ID version of the same race.
